...
function initHeaderNavLinks() {
    ...
    if (last) {
        el.href = `index.php?page=user&view=read-book&id=${last}`;
        el.style.display = '';
    }
}
...
function updateBookDetailHeaderNav(bookId, showLecture) {
    ...
    if (!navL) return;
    if (showLecture) {
        navL.href = `index.php?page=user&view=read-book&id=${bookId}`;
        navL.style.display = '';
    } else {
...
function initLampOfKnowledgeSection() {
    ...
    function update() {
        ...
        if (lumoThumb) {
            lumoThumb.src = isWorried ? 'view/user_page/assets/lumo-worried.png' : 'view/user_page/assets/lumo-happy.png';
        }
    }
...
function initChatbot() {
    ...
    function appendMsg(role, text) {
        ...
        if (!isUser) div.innerHTML = `<img src="view/user_page/assets/lumo-happy.png" alt="">`;
...
function buildCatalogBookCardHTML(book, index) {
    ...
    return `
    <div class="catalog-book-card" style="animation-delay:${delay}s" onclick="nav('index.php?page=user&view=book-detail&id=${book.id}')">
...
function buildBookCardHTML(book, index, flip = true) {
    ...
    if (flip) {
        return `
    <div class="book-card-container animate-float-up" style="animation-delay:${index * 0.05}s" onclick="nav('index.php?page=user&view=book-detail&id=${book.id}')">
...
function initAuth() {
    ...
    form.addEventListener('submit', e => { e.preventDefault(); nav('index.php?page=user'); });
}
    ...
    for (let i = 0; i < bgTileCount; i++) {
        ...
        bgHTML += `
            <div class="sm-bg-tile" style="position:absolute;top:0;left:${i * bgTileWidth}px;width:${bgTileWidth}px;height:${mapHeight}px;">
                <img src="view/user_page/assets/scholarmap2.png" alt="" draggable="false" style="width:100%;height:100%;object-fit:cover;opacity:0.9;transform:${isMirrored ? 'scaleX(-1)' : 'none'};filter:saturate(0.95) brightness(0.85);">
...
function initProfile() {
    ...
    if (libGrid) {
        ...
        <a href="index.php?page=user" class="btn-primary empty-library-cta">Browse the catalog ✦</a>
      </div>`;
        } else {
          ...
          <span class="genre-tag" style="${c.css}">${ub.book.genre}</span>
            ...
            libGrid.querySelectorAll('.book-card-static[data-book-id]').forEach(el => {
                el.addEventListener('click', () => {
                    ...
                    if (bid) nav(`index.php?page=user&view=book-detail&id=${bid}`);
                });
            });
        }
    }
...
function initBookDetail() {
    ...
    if (!book) {
      ...
      <a href="index.php?page=user" class="btn-outline">Return Home</a>
    </div>`;
        return;
    }
  ...
  <section class="community-section">
    ...
    readCta?.addEventListener('click', () => nav(`index.php?page=user&view=read-book&id=${book.id}`));
...
function initReadBook() {
    ...
    if (!book) {
      ...
      <a href="index.php?page=user" class="btn-primary">Return Home</a>
    </div>`;
        return;
    }
